...
export function Navbar() {
    ...
    // Close the desktop dropdown with Escape (WCAG 1.4.13) and on route change.
    ...
    // Mobile menu overlay: Escape to close, body scroll locked, focus moved in
    // on open and returned to the hamburger on close, Tab kept inside (2.4.3).
    ...
    return (
        <>
            <motion.nav
                aria-label="Main"
                variants={{
                    // visibility:hidden after the slide-out so keyboard focus
                    // can't land inside an invisible navbar (WCAG 2.4.11).
                ...
                <div className={`backdrop-blur-xl border rounded-full px-4 py-2 lg:px-6 lg:py-3 flex items-center justify-between gap-6 lg:gap-8 transition-all duration-300 max-w-4xl lg:max-w-5xl w-full ${scrolled ? "bg-white/80 border-black/5 shadow-2xl shadow-black/5" : "bg-white/5 border-white/10 shadow-2xl shadow-black/20"}`}>
                    ...
                    <div className={`hidden md:flex items-center rounded-full px-1 p-1 border transition-all duration-300 ${scrolled ? "bg-black/5 border-black/5" : "bg-white/5 border-white/5"}`}>
                        ...
                        <div
                            ...
                            onBlur={(e) => {
                                // Close once keyboard focus moves past the menu
                ...
                {mobileMenuOpen && (
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center gap-5 max-h-[80vh] overflow-y-auto px-6">
                            ...
                            <span className="text-xs uppercase tracking-[0.3em] text-white/60 mb-1">Services</span>
                            {services.map((s) => (
                                <Link
                                    key={s.href}
                                    to={s.href}
